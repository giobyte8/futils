...
from ..base_command import Command, RichConsoleLogger
...
class FixNameCmd(Command):
    """Command to rename movie files in a given directory"""

    def __init__(self, src_dir: str, logger=None):
        super().__init__('fix-name', logger or RichConsoleLogger())
        self.src_dir = src_dir

    def execute(self):
        if not is_dir(self.src_dir):
            self.logger.error(
                'Provided dir does not exists. {}'.format(self.src_dir)
            )
            return None

        rename_order = RenameOrder(self.src_dir)
